...
def upsample_mesh(vertices, normals, faces, displacement_map, texture_map, dense_template):
    ...
    # valid_pixel_ids = valid_pixel_ids*16
    y_coords = y_coords*4
    x_coords = x_coords*4

    uv_coords = np.zeros((59315,2))

    uv_coords[:,0] = x_coords[valid_pixel_ids]/1024
    uv_coords[:,1] = 1-y_coords[valid_pixel_ids]/1024
    ...
    dense_vertices = pixel_3d_points
...
def write_obj(obj_name,
    ...
    if 'detail' in obj_name:
        mtl_name = obj_name.replace('_detail.obj', '.mtl')
    else:
...
def plot_kpts(image, kpts, color = 'r'):
    ...
    for i in range(kpts.shape[0]):
        ...
        ##  PLOT KEYPOINTS  ##
        # if i in [36,39,42,45,33,60,64]:
        #     org = (int(st[0])+10, int(st[1]))
