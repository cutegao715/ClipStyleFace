...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        if rasterizer_type == 'pytorch3d':
            ...
            # self.uv_rasterizer = Pytorch3dRasterizer(1024)
            ...
            # self.uv_rasterizer = StandardRasterizer(1024)
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, h=None, w=None, light_type='point', background=None):
        ...
        if background is not None:
            # print(images.shape, alpha_images.shape, background.shape)
            background = torchvision.transforms.Resize(512)(background)
    ...
    def world2uv_custom(self, vertices, tex_size=None, faces=None, uvcoords=None, uvfaces=None):
        '''
        warp vertices from world space to uv space
        vertices: [bz, V, 3]
        uv_vertices: [bz, 3, h, w]
        ...
        if uvcoords is None and uvfaces is None and faces is None:
            face_vertices = util.face_vertices(vertices, self.faces.expand(batch_size, -1, -1))    # torch.Size([1, 9976, 3, 3])
            uv_vertices = self.uv_rasterizer(self.uvcoords.expand(batch_size, -1, -1), self.uvfaces.expand(batch_size, -1, -1), face_vertices)[:, :3]

        else:
            face_vertices = util.face_vertices(vertices, faces.expand(batch_size, -1, -1))
            uv_vertices = self.uv_rasterizer(uvcoords.expand(batch_size, -1, -1), uvfaces.expand(batch_size, -1, -1), face_vertices)[:, :3]
            
        if tex_size is not None:
            uv_vertices = torchvision.transforms.Resize(tex_size)(uv_vertices)
