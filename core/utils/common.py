...
def compose_text_with_templates(text, templates=("A photo of {}", )):
...
@torch.no_grad()
def encode_text(model, text, config):
    tokens = clip.tokenize(text).to(config.training.device)
    text_features = model.encode_text(tokens).detach()
    text_features /= text_features.norm(dim=-1, keepdim=True)
    return text_features


def clip_encode_image(model, image, preprocess):
    image_features = model.encode_image(preprocess(image))
    image_features /= image_features.clone().norm(dim=-1, keepdim=True)
    return image_features


def determine_opt_layers2(
    verts,
    trans_verts,
    render,
    source_generator,
    ...
    id_loss_fn = lossfunc.VGGFace2Loss(pretrained_model='./data/resnet50_ft_weight.pkl')
    sample_z = torch.randn(auto_layer_batch, 512, device=device)
    mse_loss = torch.nn.MSELoss()
    albedo_generator.eval()
    with torch.no_grad():
        _, initial_w_codes = source_generator([sample_z], truncation=0.5)
    # initial_w_codes = initial_w_codes[0].unsqueeze(1).repeat(1, source_generator.generator.module.n_latent, 1)
    ...
    verts = verts.repeat(auto_layer_batch, 1, 1)
    trans_verts = trans_verts.repeat(auto_layer_batch, 1, 1)

    for _ in range(auto_layer_iters):

        generated_from_w, _ = albedo_generator(
            [w_codes],
            input_is_latent=True
        )
        torchvision.utils.save_image(generated_from_w, 'texture.png')
        ops = render(verts, trans_verts, generated_from_w, h=config.render.img_size,
                                   w=config.render.img_size)
        render_img = ops['images']
        torchvision.utils.save_image(render_img, 'test.png')
        # w_loss = clip_loss.global_clip_loss(generated_from_w, clip_loss.target_embeddings[target_class])
        w_loss = 0.0
        w_loss += 0.5 * id_loss_fn(generated_from_w, texture)
        w_loss += mse_loss(generated_from_w * mask, texture * mask)
        for visual_encoder_key, (model, preprocess) in clip_loss.items():
            trg_encoded = clip_encode_image(model, render_img, preprocess)
            trg_domain_emb = encode_text(model, config.training.target_class, config)
            w_loss += (1.0 - torch.nn.functional.cosine_similarity(trg_encoded, trg_domain_emb).mean())
        print(w_loss)
        print(w_codes[0])
