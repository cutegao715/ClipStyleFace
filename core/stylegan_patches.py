...
class WeightModulationPatch(Patch):
    def __init__(self, conv_weight: torch.Tensor):
        super().__init__()
        self.shape = conv_weight.shape
        self.register_buffer('ones', torch.ones(self.shape))
        _, self.c_out, self.c_in = self.shape

    def forward(self, weight, offsets):
        raise NotImplementedError()

@modulation_patches.add_to_registry("w_delta")
class ChannnelWeightDelta(WeightModulationPatch):
    def forward(self, weight, offsets):
        mult = self.ones + offsets['shift']
        return weight * mult

@modulation_patches.add_to_registry("s_delta")
class ChannelStyleDelta(BaseModulationPatch):
    def forward(self, weight, offsets):
        mult = self.ones + offsets['shift']
        return weight * mult
