...
class OffsetsTunningGenerator(torch.nn.Module):
    def __init__(self, img_size=1024, latent_size=512, map_layers=8,
                 channel_multiplier=2, device='cuda:0', albedo_on=False, checkpoint_path=None):
        super().__init__()
        ...
        if checkpoint_path is not None:
            if not albedo_on:
                ...
                self.generator.load_state_dict(checkpoint["g"], strict=True)
                self.generator.eval()
                with torch.no_grad():
                ...
                self.generator.eval()
                with torch.no_grad():
                    self.mean_latent = self.generator.module.mean_latent(4096)
    ...
    def patch_layers(self, patch_key):
        ...
        if patch_key in decomposition_patches:
            ...
            raise ValueError(
                f'''
                Incorrect patch_key. Got {patch_key}, possible are { {decomposition_patches}, {modulation_patches}, {style_patches} }
                '''
            )
        return self

    def _patch_modconv_key(self, patch_key, mod_conv_class):
        self.generator.module.conv1.conv = mod_conv_class(
            patch_key, self.generator.module.conv1.conv
        )

        for conv_layer_ix in range(len(self.generator.module.convs)):
            self.generator.module.convs[conv_layer_ix].conv = mod_conv_class(
                patch_key, self.generator.module.convs[conv_layer_ix].conv
            )
    ...
    def style(self, styles):
        ...
        """
        styles = [self.generator.module.style(s) for s in styles]
        return styles
