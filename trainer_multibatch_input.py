...
from core.utils.train_log import StreamingMeans, Timer
...
from utils_3d import batch_orth_proj, random_cam, fix_cam
...
class BaseDomainAdaptationTrainer:
        ...
        if self.config.training.patch_key == "original":
            ...
            self.optimizer = torch.optim.Adam(
                [{'params': self.trainable.parameters(), 'lr':0.01},
                 {'params': self.deformnet.parameters(), 'lr':0.001}],
    ...
    def save_models(self):
        os.makedirs(os.path.join(self.config.training.checkpoint_dir, f"/{self.config.training.domain}"), exist_ok=True)
        models_dict = get_trainable_model_state(
            self.config, self.trainable.state_dict()
        )
        torch.save(self.deformnet.state_dict(), os.path.join(self.config.training.checkpoint_dir, f"/{self.config.training.domain}/deformnet_{self.current_step}.pt"))
        models_dict.update(self.ckpt_info())
        torch.save(models_dict, str(
            os.path.join(self.config.training.checkpoint_dir, f"/{self.config.training.domain}/trainable_{self.current_step}.pt")
        ))
    ...
    def _log_target_images(self, epoch):
        self.trainable.eval()
        self.deformnet.eval()
        background = torch.ones(1, 3, 512, 512).to(self.device)
        for i, batch in enumerate(self.val_loader):
            # frozen_img, latents, verts, texture, ind, _ = batch
            frozen_img, latents, verts, texture, ind = batch
            ...
            with torch.no_grad():
                ...
                up_trans_verts = batch_orth_proj(up_verts, cam)
                up_trans_verts[:, :, 1:] = - up_trans_verts[:, :, 1:]

            azims = torch.linspace(-np.pi / 6, np.pi / 6, 3)  # since 0 =360 dont include last element
            elevs = torch.linspace(-np.pi / 4, np.pi / 4, 7)

            os.makedirs(f"{self.config.training.results_save_dir}/{self.config.training.domain}/{ind}/{epoch}/",
                        exist_ok=True)
            torchvision.utils.save_image(trainable_img[0],
                                         f"{self.config.training.results_save_dir}/{self.config.training.domain}/{ind}/{epoch}/texture.png")

            for j in range(len(azims)):
                for k in range(len(elevs)):
                    cam = torch.tensor([0.55, 0.0, 0.0]).to(self.device)
                    m = j * len(elevs) + k
                    up_trans_verts = fix_cam(up_verts[0], elevs[k], azims[j])
                    up_trans_verts = batch_orth_proj(up_trans_verts, cam)
                    up_trans_verts[:, :, 1:] = - up_trans_verts[:, :, 1:]
                    with torch.no_grad():
                        ops = self.render(up_verts, up_trans_verts, trainable_img, h=512,
                                          w=512, background=background)
                    torchvision.utils.save_image(ops['images'],
                                                 f"{self.config.training.results_save_dir}/{self.config.training.domain}/{ind}/{epoch}/{m}th_view.jpg")
            torch.save(verts[0],
                       f"{self.config.training.results_save_dir}/{self.config.training.domain}/{ind}/{epoch}/up_verts.pt")
            save_obj(
                f"{self.config.training.results_save_dir}/{self.config.training.domain}/{ind}/{epoch}/shape.obj",
                up_verts[0], self.faces[0])

    def train_loop(self):
        self.all_to_device(self.device)

        iter_info = StreamingMeans()
        train_data = ValDataset(self.device)
        # train_data = ValDataset(self.device, self.config.training.domain)
        train_loader = DataLoader(train_data, batch_size=self.config.training.batch_size, shuffle=True, drop_last=True, num_workers=0)
        val_loader = DataLoader(train_data, batch_size=1, shuffle=False, num_workers=0)

        self.val_loader = val_loader
        index = 0
        for epoch in range(self.config.exp.epochs):
            self.save_models()
            for i, batch in enumerate(train_loader):
                self.current_step += 1
                with Timer(iter_info, "train_iter"):
                    self.train_step(iter_info, batch)
                index += 1
            self._log_target_images(epoch)
        wandb.finish()
    ...
    def train_step(self, iter_info, batch):
        self.trainable.train()
        self.deformnet.train()

        batch = self.calc_batch(batch)
...
class TextDrivenSingleDomainAdaptationTrainer(SingleDomainAdaptationTrainer):
    ...
    def calc_batch(self, batch):
        ...
        frozen_img, latents, verts, texture, _ = batch
        # frozen_img, latents, verts, texture, _, neg_img = batch
        trainable_img, offsets = self.forward_trainable([latents], input_is_latent=True)
        ...
        ops = self.render(up_verts, up_trans_verts, trainable_img, h=self.config.render.img_size,
        ...
        if self.current_step > 30:
            for visual_encoder_key, (model, preprocess) in self.batch_generators.items():
                trg_encoded = self.clip_encode_image(model, rendered_img, preprocess)
                src_encoded = self.clip_encode_image(model, neg_img, preprocess)
        ...
        # torchvision.utils.save_image(vis_img, f"ablation7/vis_img_{self.current_step}.png", nrow=2)
        return {
            'clip_data': clip_data,
            'rec_data': None,
            'offsets': offsets
